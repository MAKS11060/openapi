...
//////////////// Achievements
doc
  .addPath('/api/achievements') //
  .parameter('query', 'user_id', (t) => t.schema(userID).required())
  .get((t) => {
    t.tag('achievements')
    t.response(200, (t) => {
...
doc
  .addPath('/api/constants/user_rate') //
  .get((t) => {
    t.tag('constants')
    t.response(200, (t) => {
      t.content(
