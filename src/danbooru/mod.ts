...
doc.addPath('/tags.json')
  .get((t) => {
    ...
    t.parameter(limitQueryParam)
    t.parameter(pageQueryParam)
  ...
  .post((t) => {
    t.tag('tags')
    t.operationId('list_tags_use_post')

    t.parameter('header', 'X-HTTP-Method-Override', (t) => {
      t.required().schema(z.literal('get'))
    })
